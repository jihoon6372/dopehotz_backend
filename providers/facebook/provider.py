...
class FacebookProvider(OAuth2Provider):
    ...
    def sociallogin_from_response(self, request, response):
        sociallogin = super(FacebookProvider, self).sociallogin_from_response(request, response)
        if not sociallogin.user.username:
            sociallogin.user.username = 'facebook_%s' % sociallogin.account.extra_data['id']
        return sociallogin
